<form action="{{ route('leave_application.update', $leaveApplication->id) }}" method="POST">@csrf
                        ...
                        @foreach ($departments as $department)
                            <optgroup label="{{ $department?->name }}">
                                @foreach ($department->employees as $row)
                                    <option value="{{ $row->id }}"
                                        {{ $leaveApplication->employee_id == $row->id ? 'selected' : '' }}>
                                        {{ $row->name }} </option>
                                @endforeach
                        @endforeach
                    </select>
                    ...
                    <select class="form-select select2 form-control-sm select2" id="leave_type_id" name="leave_type_id"
                        ...
                        @foreach ($leaveTypes as $leaveType)
                            <option value="{{ $leaveType->id }}"
                                {{ $leaveApplication->leave_type_id == $leaveType->id ? 'selected' : '' }}>
                ...
                <div class="table-responsive">
                    <table class="table table-hover table-responsive table-bordered leave-table">
                        <tbody>
                            ...
                            <tr>
                                <th>Leave Table</th>
                                <td>{{ $leaveApplication->employee?->leaveTable?->title }}</td>
                            </tr>
                            <tr>
                                <th>Taken</th>
                                <td>0</td>
                ...
                <div class="table_leave_div">
                    <table width="100%" border="0" cellspacing="0" cellpadding="0"
                        ...
                        <tbody>
                            ...
                            @if ($leaveApplication->employee?->leaveTable?->leaveTableDetails)
                                @foreach ($leaveApplication->employee?->leaveTable?->leaveTableDetails as $leaveTableDetail)
                                    <tr>
                                        <td>Annual</td>
